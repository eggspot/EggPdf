...
public class WhiteSpaceTests
{
    ...
    [Fact]
    public void MixedInlineContent_CrlfAfterInlineSibling_ProducesNoRawControlCharacters()
    {
        // Reproduces a real-world bug: an inline sibling (<strong>) followed by a text
        // node containing a literal CRLF + indentation (common when HTML is generated by
        // templates that wrap long source lines for readability). TrimHtmlText only trims
        // the edges of that text node — it deliberately doesn't collapse internal \r\n —
        // so word-splitting for this "mixed inline content" path must also treat \r/\n as
        // delimiters, or a raw control character reaches the painted text and renders as
        // a tofu/.notdef box (no font has a glyph for a bare control character).
        var html = "<p>Foo <strong>bar</strong> baz)\r\n    qux</p>";
        var root = LayoutTestHelper.Layout(html, 600, 800);

        var p = root.FindByTag("p");
        p.Should().NotBeNull();

        var allText = string.Join("", p!.Children.Select(c => c.Text ?? ""));
        allText.Should().NotContain("\r", "a raw CR must not reach a painted text box");
        allText.Should().NotContain("\n", "a raw LF must not reach a painted text box");
        allText.Should().Contain("qux");
    }
