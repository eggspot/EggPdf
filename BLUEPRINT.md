...
- `StructureTreeBuilder.cs` (new, in `src/EggPdf/`) walks the *pre-pagination* `LayoutBox` tree (the same tree `PageFragmenter.CollectPaintableBoxes` walks) and builds a parallel `PdfStructureElement` tree, mapping the tags this MVP tags -- `h1`-`h6`, `p`, `table`/`tr`/`th`/`td`, `ul`/`ol`/`li` (wrapped in `LBody` per ISO 14289-1 7.2-17..20), `img` (with `alt`), `a` -- to PDF standard structure types. Every other reachable box (untagged wrappers, anonymous text-run fragments from line wrapping) maps to its nearest tagged ancestor, so nothing is dropped.
...
- `PdfDocument.Tagging.cs` (new) writes every `StructElem` object, `StructTreeRoot`, and the `ParentTree` number tree (`/StructParents` per page -> MCID-indexed array of owning-element refs) that lets a reader walk from page content back to the tree. `/MarkInfo`, `/ViewerPreferences /DisplayDocTitle`, `/Lang`, and `/Tabs /S` per page are added to the catalog/page dicts. The XMP writer (`PdfACompliance`) gained `pdfuaid:part` + a required `dc:title` (defaulted to "Untitled Document" rather than omitted) -- independent of `PdfAConformance`, so pure PDF/UA-1 (no PDF/A claim) and PDF/A+PDF/UA-1 combined both work through the same XMP method.
- API: `HtmlToPdf.Render(html, tagged: true)`, `Render(html, conformance, tagged: true)`, `PdfRenderOptions.Tagged`, the CLI's `--tagged` flag, and the REST API's `options.tagged` field -- wired into all four entry points from the start, not retrofitted.
...
- **Known limitations, stated plainly:** MVP tag set only (no landmark regions, no inline emphasis/strong mapping); `<a>` elements are tagged like any other content (no Link-annotation `OBJR` cross-reference); position:fixed/margin-box content (running headers/footers) isn't tagged; named page groups (`page: <name>` + `@page <name>`) throw rather than silently under-tag, since correlating structure across independently-laid-out groups wasn't attempted. `PdfTaggedStructure.cs`, the pre-existing unused scaffold, was not reused (the real design needed per-page MCID/ParentTree bookkeeping that scaffold didn't have) and has been deleted rather than left as dead code. (PDF/A-2a/3a as named `PdfAConformance` enum members -- once a genuinely open item -- are built; see the "Current status" note above.)
