...
Still not built: step (2) tagging / PDF/UA-1 (needs a full structure tree spanning the layout, paint and PDF-writer layers) -- the one remaining item from the original three (archival/e-invoicing/accessibility) the user asked to cover. `PdfTaggedStructure.cs` still exists but is not wired into the writer.
