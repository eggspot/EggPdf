...
| **PDF/A conformance** | PDF/A-1b/1a, PDF/A-2b/2u/2a, PDF/A-3b/3u/3a (with file attachments), PDF/A-4/4f/4e (PDF 2.0-based, ISO 19005-4). Output intent with a real embedded ICC profile. No encryption. All fonts embedded+subsetted. No external references. Note: PDF/A-1 has no `u` level -- only `a`/`b` -- unlike parts 2/3, which have `a`/`b`/`u` |
...
**PDF/UA-2 (ISO 14289-2:2024) is built**, targeting the parts verified with confidence against veraPDF's published UA-2 validation rules (see the `10c` PDF/UA row below for the full writeup). **WTPDF (Well-Tagged PDF)**'s Level 1 "for reuse" conformance tier -- a weaker tier than PDF/UA-2's accessibility tier, both defined by the same PDF Association WTPDF 1.0 spec -- is not modeled; EggPdf's UA-2 support targets accessibility conformance only.
...
**Current status (as of this writing):** step (1), the conformance foundation, is built for the full matrix: **PDF/A-1b, PDF/A-1a, PDF/A-2b, PDF/A-2u, PDF/A-2a, PDF/A-3b, PDF/A-3u, PDF/A-3a**. `PdfDocument.Conformance` (see `PdfDocument.PdfA.cs`) embeds a real ICC profile (`IccSrgbProfile`, a structurally valid hand-built sRGB profile -- the previous 128-byte header-only stub is gone), spec-correct XMP (`PdfACompliance`), and forces standard fonts to be embedded (`HtmlToPdf.cs`); encryption + conformance together throws, since PDF/A forbids encryption. The `u` levels needed no extra writer behavior -- every conformance level already routes fonts through CIDFont Type 2 embedding, which always carries a real `ToUnicode` CMap, so `u` only had to assert that guarantee in the XMP conformance letter. **Correction to an earlier version of this note:** it originally described a `PdfA1u` level, which was a mistake -- ISO 19005-1 (PDF/A-1) only has `a`/`b` levels, unlike parts 2/3 which have `a`/`b`/`u`; there is no PDF/A-1u. This was caught and fixed (renamed to the correct `PdfA1a`) while building the `a`-level support below, before it reached `main`. Reachable via `HtmlToPdf.Render(html, PdfAConformance.___)`, or via `PdfRenderOptions.Conformance`/`.Invoice`/`.Encryption` (all three now live on `PdfRenderOptions`, not just CSS-translatable settings -- see its updated doc comment), the CLI's `--pdfa`/`--invoice` flags, and the REST API's `options.conformance`/`options.invoice` JSON fields. This is the "Feature Parity Across Entry Points" rule in `CLAUDE.md`, added after this exact gap was flagged.

**The `a` levels (PdfA1a/2a/3a) require accessibility tagging**, since level A conformance IS PDF/A + PDF/UA-1-equivalent tagging (see step (2) below) -- `PdfDocument` throws `InvalidOperationException` if `Conformance` is an `a` level and `StructureTree` isn't set, rather than silently claim accessibility conformance without a structure tree. Reachable via `HtmlToPdf.Render(html, conformance, tagged: true)`; the CLI's `--pdfa 2a --tagged` (etc.) and REST API's `options.conformance/options.tagged` reject the combination the same way.

**ZUGFeRD/Factur-X (10g) is built for MINIMUM and EN 16931 (Comfort)**: `PdfDocument.Invoice` (see `PdfDocument.FacturX.cs`) takes a `FacturXInvoice` (seller/buyer, dates, totals, and now `LineItems: List<FacturXLineItem>` -- modeled as a strict subset of EN 16931's fields, not a MINIMUM-specific shape) and embeds its UN/CEFACT CII XML (`FacturXCiiWriter`, structure verified against reference samples at github.com/invoice-x/factur-x-ng, including `facturx/flavors/factur-x/xml/samples/en16931.xml`) as a PDF/A-3 file attachment. The profile is derived from data, not a caller-declared enum: `FacturXCiiWriter.IsEn16931(invoice) => invoice.LineItems.Count > 0`. Empty `LineItems` produces the original MINIMUM output (`urn:factur-x.eu:1p0:minimum` guideline ID, `/AFRelationship /Data`); non-empty `LineItems` produces EN 16931 output instead (`urn:cen.eu:en16931:2017` guideline ID, `/AFRelationship /Alternative` for German legal validity): each line gets its own `IncludedSupplyChainTradeLineItem` with a line-level `ApplicableTradeTax`, lines are grouped by `(VatCategoryCode, VatRatePercent)` into a header-level tax breakdown (`ComputeTotals` in `FacturXCiiWriter`), and the header monetary summation (`TaxBasisTotalAmount`/`TaxTotalAmount`/`GrandTotalAmount`/`DuePayableAmount`) is fully computed from the line items -- the invoice's own header total properties are ignored when `LineItems` is set, so caller-supplied and computed totals can never disagree. `PdfACompliance.GenerateXmpMetadata` takes the resulting level as `facturXConformanceLevel` and emits it as `<fx:ConformanceLevel>`. Setting `Invoice` without `PdfA3b`/`PdfA3u` conformance throws, matching the Encryption+Conformance guard. Reachable via `HtmlToPdf.Render(html, PdfAConformance.PdfA3b, FacturXInvoice)`, `PdfRenderOptions.Invoice`, the CLI's `--invoice` flag (zero code changes needed -- the CLI's generic `JsonSerializer.Deserialize<FacturXInvoice>` already picks up `LineItems`), or the REST API's `options.invoice.lineItems` field (new `InvoiceLineItemRequest` record in `Program.cs`). **Scoping note:** BASIC and EXTENDED are not modeled as distinct profiles -- populating `LineItems` always targets EN 16931, a strict superset of BASIC's requirements but without EXTENDED-only fields (allowances/charges, multiple deliveries, additional references). `PdfAttachment.cs`'s general-purpose attachment scaffold was not reused (Factur-X's Names-tree/AF wiring is written directly in `PdfDocument.FacturX.cs` instead) and remains unwired.

**PDF/A-1b/1a is built too**: the writer header switches to `%PDF-1.4` under PdfA1b/PdfA1a (`PdfDocument.PdfVersionHeader`), and since PDF/A-1 forbids transparency outright (ISO 19005-1, veraPDF rules 6.4-1/2/4/5: no partial opacity, no blend modes besides Normal/Compatible, no image alpha channels), `PdfDocument.ValidatePdfA1NoTransparency()` scans every page's ExtGState usage and every registered image and throws `InvalidOperationException` if any of those appear -- rather than silently dropping the CSS opacity/blend-mode/alpha-image the document asked for (wrong output) or silently emitting a mislabeled non-conformant PDF. This means PdfA1b/PdfA1a only succeed for documents that don't use those features; anything that does needs PdfA2b/2u/3b/3u instead. Verified against veraPDF's own published PDF/A Part 1 rules (github.com/veraPDF/veraPDF-validation-profiles/wiki), not just recollection.

**Step (2) tagging / PDF/UA-1 is built too**, closing out all three of the original archival/e-invoicing/accessibility asks. It genuinely spans three subsystems, not just `PdfDocument`:

- `StructureTreeBuilder.cs` (new, in `src/EggPdf/`) walks the *pre-pagination* `LayoutBox` tree(s) (the same tree(s) `PageFragmenter.CollectPaintableBoxes` walks; a list, not a single root, when named page groups are in play -- see below) and builds a parallel `PdfStructureElement` tree, mapping the tags this MVP tags -- `h1`-`h6`, `p`, `table`/`tr`/`th`/`td`, `ul`/`ol`/`li` (wrapped in `LBody` per ISO 14289-1 7.2-17..20), `img` (with `alt`), `a`, and the landmark set `nav`/`header`/`footer`/`aside`/`main`/`article`/`section` -- to PDF structure types. Every other reachable box (untagged wrappers, anonymous text-run fragments from line wrapping) maps to its nearest tagged ancestor, so nothing is dropped.
- **Landmark regions**: PDF 1.7 (what PDF/UA-1 is based on) has no native semantic-landmark structure types, so `nav`/`header`/`footer`/`aside`/`main`/`article`/`section` map to custom (non-standard) type names -- exactly the case ISO 32000-1 14.7.5's `/RoleMap` mechanism exists for. `PdfDocument.Tagging.cs`'s `LandmarkRoleMap` declares the fallback (`Nav`/`Header`/`Footer`/`Aside`/`Main` -> `Div`, `Article`/`Section` -> `Sect`) and it's always written on `StructTreeRoot` when tagged, whether or not a given landmark tag is actually used.
- **Link-annotation `OBJR` cross-referencing**: `PdfLinkAnnotation` (now `public`, not `internal`, for the same cross-assembly reason `PdfPage.BeginMarkedContent`/`EndMarkedContent` are) gained a `TaggedElement` property that `BoxPainter` sets when painting a tagged `<a>` box; `PdfDocument.WriteTo` (via new `LinkAnnotationsToStructureTree`) then allocates the annotation its own indirect object number (annotations were previously always written inline in the page dict) and a `/StructParent` key -- starting right after the last page index so it shares the ParentTree's number space with pages' `/StructParents` without colliding -- and registers an OBJR entry on the Link element via `PdfStructureElement.AddAnnotationRef`. ISO 14289-1 7.18.1 requires this: without it, a reader can tell anchor text is a link by content alone but has no structural link to the clickable annotation.
- **Named page groups + tagging together**: `StructureTreeBuilder.Build` takes a list of layout roots (one per named page group) instead of one, building a single Document root all groups attach to. The blocker was `HtmlToPdf.NamedPages.cs`'s `RenderPageGroups`, which pre-renders every group once into a throwaway `PdfDocument` just to learn its page count (for `counter(pages)`/margin boxes) before the real render -- with tagging on, that scratch pass would register real (page, MCID) pairs from its about-to-be-discarded pages against the one shared structure tree, doubling every element's content refs. Fixed by nulling `BoxPainter.StructureMap` around just the scratch/precount loop and restoring it before the real render loop.
- `BoxPainter.PaintBox` was split into a thin wrapper + `PaintBoxCore`: the wrapper checks a `[ThreadStatic] StructureMap` (set by `HtmlToPdf` before painting, mirroring the existing `CurrentPdfDoc` field) and, when the box has an entry, wraps `PaintBoxCore`'s entire output in one `BDC`/`EMC` marked-content span via new `PdfPage.BeginMarkedContent`/`EndMarkedContent` methods, then registers the (page, MCID) pair against the structure element. Because `PaintBox` is invoked once per page per box already (not a recursive walk -- see 10c's earlier notes on pagination), a box painted twice (repeating `<thead>` rows) just gets two MCR entries under the same element, which is spec-legal.
- `PdfDocument.Tagging.cs` (new) writes every `StructElem` object, `StructTreeRoot`, and the `ParentTree` number tree (`/StructParents` per page -> MCID-indexed array of owning-element refs, plus each tagged annotation's `/StructParent` key -> a direct element reference, sharing the same number space) that lets a reader walk from page content back to the tree. `/MarkInfo`, `/ViewerPreferences /DisplayDocTitle`, `/Lang`, `/Tabs /S` per page, and `/RoleMap` are added to the catalog/page dicts/StructTreeRoot. The XMP writer (`PdfACompliance`) gained `pdfuaid:part` + a required `dc:title` (defaulted to "Untitled Document" rather than omitted) -- independent of `PdfAConformance`, so pure PDF/UA-1 (no PDF/A claim) and PDF/A+PDF/UA-1 combined both work through the same XMP method.
- API: `HtmlToPdf.Render(html, tagged: true)`, `Render(html, conformance, tagged: true)`, `PdfRenderOptions.Tagged`, the CLI's `--tagged` flag, and the REST API's `options.tagged` field -- wired into all four entry points from the start, not retrofitted. No new API surface was needed for landmark regions, OBJR, or named-page-group support -- they're automatic once `tagged: true` is set.
- **Fixed along the way:** the HTML tree builder silently dropped every attribute on a literal `<html ...>` tag (a pre-existing bug unrelated to this feature, caught because `/Lang` needs `<html lang>`) -- `HtmlTreeBuilder.ProcessStartTag`'s `html` branch now applies the token's attributes to the document element.
- **Fixed: multi-word `<a>` links only tagging/cross-referencing their first word.** Root cause was in `BlockLayout.InlineRuns.cs`'s `LayoutInlineRuns`: `elementAssigned` is a one-shot gate that assigns the DOM `Element` reference to only the first per-word `LayoutBox` fragment of any inline element (`<a>`, but also `<strong>`/`<span>`/etc.) -- every later word-fragment got `Element = null`, no back-reference to its DOM node at all. This was pre-existing and independent of tagging: it also meant the plain, non-tagged `PdfPage.AddLink` rectangle for a multi-word link was already sized to only the first word. Fixed additively, without touching that gate (so background/border painting, which relies on "only the first fragment paints its own box", is untouched): a new `InlineElementSpan` (in `EggPdf.Layout`) is shared by every word-fragment on one line of one inline element -- `LayoutInlineRuns` unions each new word's rect into the current line's span, starting a fresh span when the line changes (a link that wraps gets one span, one Link element, and one annotation per line, since a PDF rectangle can't itself wrap) -- and `LayoutBox.InlineSpan` exposes it. `StructureTreeBuilder.BuildRecursive` now keys its per-fragment lookup off `box.InlineSpan` when present (falling back to `box.TagName` otherwise), caching one `PdfStructureElement` per span so every fragment on the line reuses it instead of creating one Link per word. `BoxPainter`'s link-painting code does the same for annotations: it unions the span's rect into page-local coordinates (via the offset between the span's and the current fragment's stored X/Y -- both are in the same pre-pagination space, and a span never crosses a page since pagination only breaks between lines) and calls `PdfPage.AddLink` once per span, caching the created `PdfLinkAnnotation` on `InlineElementSpan.PaintTag` (an untyped slot, so the Layout-layer type doesn't need to reference PDF writer types) so later fragments on the same line don't create a second, overlapping annotation.
- **Known limitations, stated plainly:** no inline emphasis/strong mapping (`em`/`strong` aren't given a distinct structure type -- they fall through to their nearest tagged ancestor like any other unrecognized tag); position:fixed/margin-box content (running headers/footers) isn't tagged. (PDF/A-2a/3a as named `PdfAConformance` enum members -- once a genuinely open item -- are built; see the "Current status" note above.)

**PDF/UA-2 (ISO 14289-2:2024) is built as a real, separately-testable conformance path**, reusing all of PDF/UA-1's StructTreeRoot/ParentTree/MCID machinery rather than forking it, plus the parts of PDF 2.0's own requirements verified with confidence against veraPDF's published UA-2 validation rules before writing any code (this is a newer, less mature standard than PDF/UA-1 -- the same caution that caught the earlier `PdfA1u` mistake applied here up front instead of after the fact):

- New `PdfUaVersion` enum (`Ua1` default, `Ua2`) on `PdfDocument.UaVersion`, only meaningful when `StructureTree` is set. `PdfVersionHeader` switches to `%PDF-2.0` for `Ua2` (checked before the PDF/A-1-implies-`%PDF-1.4` case, since UA-2 always wins when both could apply -- though see the next point, they can't actually coexist).
- `WriteTo()` throws if `UaVersion == Ua2` and `Conformance != null` -- there's no defined joint PDF/A + PDF/UA-2 standard (unlike PDF/UA-1's level-A conformance), so combining them would silently claim a conformance that doesn't exist. The CLI's `--ua-version 2` + `--pdfa` combination is rejected the same way before rendering, mirroring the level-A+`--tagged` validation pattern.
- `PdfDocument.Tagging.cs` allocates and writes a `/Type /Namespace /NS (http://iso.org/pdf2/ssn)` object (the PDF 2.0 standard structure namespace, ISO 32000-2) when `UaVersion == Ua2`, declares it on `StructTreeRoot` via `/Namespaces [obj 0 R]`, and every `StructElem` gets `/NS {obj} 0 R` -- satisfying veraPDF UA-2 rule 8.2.4-1 ("every structure element shall belong to... the PDF 2.0 namespace") for the structure types EggPdf actually emits. `/RoleMap` (UA-1's flat custom-type fallback) is omitted for UA-2 rather than also emitted, since UA-2's landmark handling (next point) makes it unnecessary.
- **Landmark regions under UA-2 resolve straight to their standard fallback type** (`Div`/`Sect`) instead of the custom name (`Nav`/`Header`/etc.) UA-1 uses + `/RoleMap`. PDF 2.0's actual role-mapping mechanism for custom types is `/RoleMapNS` -- namespace-scoped, and per veraPDF rules 8.2.4-3/-4, forbids mapping within the same namespace or mapping a standard type to a non-standard one, meaning a custom landmark type would need its own separate declared namespace to map from. That's a materially more involved mechanism than UA-1's flat `/RoleMap`, and my source research on PDF/UA-2 (a research pass done deliberately before writing any code, given the standard's immaturity) couldn't independently confirm which HTML5 landmark types PDF 2.0 does vs. doesn't add as genuinely standard (only "Aside" was confirmed as newly standard; Nav/Header/Footer/Main/Article/Section weren't). Resolving straight to `Div`/`Sect` sidesteps that unresolved question entirely and stays correct under either interpretation, at the cost of losing the semantic landmark distinction specifically under UA-2 (`StructureTreeBuilder`'s `Ua2LandmarkFallback`, threaded through `Build`/`BuildRecursive` via a `uaVersion` parameter).
- XMP gains `pdfuaid:rev` (`2024`) alongside `pdfuaid:part` (`2` instead of `1`) for UA-2, per the PDF Association's own "future-proofing XMP identification" guidance (`PdfACompliance.GenerateXmpMetadata`'s new `isUa2` parameter).
- API: `HtmlToPdf.Render(html, tagged: true, PdfUaVersion.Ua2)` and the matching `RenderAsync`/conformance-combo overloads, `PdfRenderOptions.UaVersion`, the CLI's `--ua-version <1|2>` flag (requires `--tagged`), and the REST API's `options.uaVersion` field (`"Ua1"`/`"Ua2"`) -- wired into all four entry points in the same change.
- **Deliberately NOT attempted, stated plainly rather than guessed at:** structure destinations for internal links (ISO 14289-2 8.8-1 requires intra-document link/outline destinations to be structure destinations, not page destinations -- EggPdf doesn't support internal anchor links at all yet, tagged or not, so this is orthogonal, larger, unrelated work); `/Headers` on table cells or ISO 32000-2 §14.8.5.7's algorithmic-determination alternative (EggPdf's tables keep using the same `<th scope>`-derived `/Scope` attribute UA-1 uses); full `/RoleMapNS` support for genuinely custom (non-fallback-able) structure types; and dropping `/Info` in favor of XMP-only metadata -- sources conflicted on whether PDF/UA-2 actually forbids `/Info` (vs. just deprecating it in PDF 2.0 generally), and removing it would be a large, hard-to-reverse-safely change touching nearly every existing PDF-writing code path, so `/Info` is kept as the conservative choice pending clearer verification, not a considered compliance decision.
- Verified end-to-end: `%PDF-2.0` header, `/Namespaces`/`/NS` wiring, and `pdfuaid:part`/`pdfuaid:rev` confirmed both via the unit/E2E test suite and by rendering through the CLI (`--tagged --ua-version 2`) and REST API (`options: {tagged: true, uaVersion: "Ua2"}`) and inspecting the actual output bytes -- not just that rendering didn't crash. The PDF/A+UA-2 rejection was also confirmed live via the REST API (`400` with the expected message).
...
| **ZUGFeRD / Factur-X** | Built for MINIMUM and EN 16931/Comfort profiles (`PdfDocument.Invoice`, see 10c): PDF/A-3 with embedded CII XML invoice; empty `LineItems` -> MINIMUM (`/AFRelationship /Data`), non-empty `LineItems` -> EN 16931 (line-level + grouped header tax breakdown, computed totals, `/AFRelationship /Alternative`), Factur-X XMP extension schema. BASIC/EXTENDED are not modeled as distinct profiles. This is the European e-invoicing standard |
...
**PDF/A archival conformance -- done (depends on the foundation above):**
- PDF/A-2b/2u and PDF/A-3b/3u (PDF 1.7-based) -- primary target, most commonly requested
- PDF/A-1b/1a (legacy, PDF 1.4-based): header switches to `%PDF-1.4`; transparency (opacity, blend modes, image alpha) is forbidden outright under PDF/A-1, so rendering throws rather than silently dropping it or shipping non-conformant output. PDF/A-1 has no `u` level (only `a`/`b`) -- unlike parts 2/3
- `u`-level: every embedded font already carries a correct `ToUnicode` CMap, so no separate audit was needed
- `a`-level (PdfA1a/2a/3a): requires `StructureTree` to be set (tagging) -- throws otherwise
...
**File attachments (ZUGFeRD / Factur-X MINIMUM + EN 16931 profiles -- built, depends on PDF/A-3 above):**
- ZUGFeRD/Factur-X support is done for MINIMUM and EN 16931 (Comfort): `HtmlToPdf.Render(html, PdfAConformance.PdfA3b, FacturXInvoice)` embeds the CII XML as a PDF/A-3 attachment. The profile is derived from `FacturXInvoice.LineItems`, not a caller-declared enum: empty -> MINIMUM (`/AFRelationship /Data`), non-empty -> EN 16931 (line-level + header-grouped tax breakdown, computed totals, `/AFRelationship /Alternative`)
- Not built: BASIC/EXTENDED as *distinct* profiles (EN 16931 is a strict superset of BASIC's requirements but doesn't add EXTENDED-only fields like allowances/charges or multiple deliveries), and a general-purpose attachments API (`PdfAttachment.cs`'s collection scaffold exists but isn't wired into the writer -- Factur-X's embedding was written directly rather than through it)
...
- MINIMUM and EN 16931 (Comfort) profiles are built (Phase 13, `FacturXInvoice.LineItems`-driven); remaining work is BASIC and EXTENDED as *distinct* modeled profiles (EXTENDED needs allowances/charges, multiple deliveries, additional references beyond what the current line-item model carries)
...
- File attachments embed correctly (ZUGFeRD/Factur-X MINIMUM and EN 16931 profile XML validates against a real Factur-X validator -- not run yet, only checked against reference sample structure)
