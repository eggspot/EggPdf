...
- **Discovered but NOT fixed, flagged plainly:** a multi-word `<a>` (e.g. "Click here") only tags/cross-references its *first word* as the Link element. Root cause is in `BlockLayout.InlineRuns.cs`'s `LayoutInlineRuns`: `elementAssigned` is a one-shot gate that assigns the DOM `Element` reference to only the first per-word `LayoutBox` fragment of any inline element (`<a>`, but also `<strong>`/`<span>`/etc.) -- every later word-fragment gets `Element = null`, i.e. no back-reference to its DOM node at all, not just an untagged one. This is a pre-existing bug independent of tagging: it also means the plain, non-tagged `PdfPage.AddLink` rectangle for a multi-word link is already sized to only the first word, so multi-word links are under-clickable in every EggPdf render today, not just tagged ones. Fixing it properly (deciding whether every word-fragment should carry the DOM `Element`, and auditing what currently relies on "only the first fragment has one") is its own focused fix with its own test coverage, not bundled into this batch -- tracked here and in `site/compliance.html`'s known-limitations note instead of silently left undiscoverable.
