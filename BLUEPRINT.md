...
- **Known limitations, stated plainly:** MVP tag set only (no landmark regions, no inline emphasis/strong mapping); `<a>` elements are tagged like any other content (no Link-annotation `OBJR` cross-reference); position:fixed/margin-box content (running headers/footers) isn't tagged; named page groups (`page: <name>` + `@page <name>`) throw rather than silently under-tag, since correlating structure across independently-laid-out groups wasn't attempted; PDF/A-2a/3a (PDF/A + PDF/UA-1 combined conformance *levels*, as opposed to just combining the two XMP schemas, which already works) still needs the `PdfAConformance` enum extended with `PdfA2a`/`PdfA3a` members -- not done, since nothing currently requests it. `PdfTaggedStructure.cs`, the pre-existing unused scaffold, was not reused (the real design needed per-page MCID/ParentTree bookkeeping that scaffold didn't have) and has been deleted rather than left as dead code.
...
- PDF/A-1b/1u (legacy, PDF 1.4-based): header switches to `%PDF-1.4`; transparency (opacity, blend modes, image alpha) is forbidden outright under PDF/A-1, so rendering throws rather than silently dropping it or shipping non-conformant output
