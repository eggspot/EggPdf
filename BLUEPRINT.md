...
**Current status (as of this writing):** step (1), the conformance foundation, is built for **PDF/A-2b and PDF/A-3b**: `PdfDocument.Conformance` (see `PdfDocument.PdfA.cs`) embeds a real ICC profile (`IccSrgbProfile`, a structurally valid hand-built sRGB profile -- the previous 128-byte header-only stub is gone), spec-correct XMP (`PdfACompliance`), and forces standard fonts to be embedded (`HtmlToPdf.cs`); encryption + conformance together throws, since PDF/A forbids encryption. Reachable via `HtmlToPdf.Render(html, PdfAConformance.PdfA2b/PdfA3b)` only -- not yet wired into `PdfRenderOptions`, the CLI, or the REST API. Still not built: PDF/A-1b/1u (blocked on the PDF-version-selection feature, since our header is fixed at PDF 1.7), `u`-level `ToUnicode` auditing, step (2) tagging, PDF/UA-1, and ZUGFeRD/Factur-X (10g). `PdfTaggedStructure.cs` and `PdfAttachment.cs` still exist but are not wired into the writer -- that's step (2) and the file-attachments work below, not assumed complete.
