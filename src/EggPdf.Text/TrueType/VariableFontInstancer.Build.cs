...
public static partial class VariableFontInstancer
{
    ...
    private sealed class InstanceContext
    {
        ...
        private void BuildSimpleGlyph(int gid, int g, int contours, int xMin0, int lsb0, InstancedGlyph result)
        {
            ...
            result.Data = WriteSimple(ends, nx, ny, flags, xMin, yMin, xMax, yMax);
        }

        private static byte[] WriteSimple(int[] ends, int[] xs, int[] ys, byte[] originalFlags, int xMin, int yMin, int xMax, int yMax)
        {
            int n = xs.Length;
            ...
            U16w(0); // no instructions: the outline moved, so the original hinting no longer applies

            for (int i = 0; i < n; i++) w.Add((byte)(originalFlags[i] & 0x01)); // on-curve bit only; 16-bit coordinates
            int prev = 0;
        ...
        private static void PutI16(byte[] a, int at, int v) { a[at] = (byte)(v >> 8); a[at + 1] = (byte)v; }
        ...
        {
            "fvar", "gvar", "avar", "HVAR", "VVAR", "MVAR", "STAT", "cvar", "DSIG", "hdmx", "LTSH", "VDMX",
        };

        private byte[] Assemble()
        {
            // glyf + long-format loca
            var glyf = new List<byte>();
            var loca = new byte[(_numGlyphs + 1) * 4];
            for (int g = 0; g < _numGlyphs; g++)
            {
                PutU32(loca, g * 4, (uint)glyf.Count);
                var data = _glyphs[g]!.Data;
                glyf.AddRange(data);
                while (glyf.Count % 4 != 0) glyf.Add(0);
            }
            PutU32(loca, _numGlyphs * 4, (uint)glyf.Count);

            // hmtx keeps its layout; advances of glyphs past numberOfHMetrics stay shared
            var hmtx = new byte[_numHMetrics * 4 + (_numGlyphs - _numHMetrics) * 2];
            int advanceMax = 0;
            for (int g = 0; g < _numGlyphs; g++)
            {
                var glyph = _glyphs[g]!;
                ...
                {
                    PutI16(hmtx, _numHMetrics * 4 + (g - _numHMetrics) * 2, glyph.LeftSideBearing);
                }
            }

            var replaced = new Dictionary<string, byte[]> { ["glyf"] = glyf.ToArray(), ["loca"] = loca, ["hmtx"] = hmtx };

            var head = Slice(_s.Offset("head"), _s.Length("head"));
            ...
            var hhea = Slice(_s.Offset("hhea"), _s.Length("hhea"));
            ...
            foreach (var tag in _s.Tags)
            {
                ...
                tables[tag] = replaced.TryGetValue(tag, out var body) ? body : Slice(_s.Offset(tag), _s.Length(tag));
            }
        ...
        private byte[] Slice(int offset, int length)
        {
            ...
            Array.Copy(_d, offset, b, 0, length);
            return b;
        }

        private static void PutU32(byte[] a, int at, uint v)
        {
        ...
        private static byte[] WriteFont(SortedDictionary<string, byte[]> tables)
        {
